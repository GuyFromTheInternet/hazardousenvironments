package com.abandonsearch.hazardgrid.ui.theme

import androidx.compose.ui.graphics.Color

val Primary = Color(0xFFC2D9FF)
val OnPrimary = Color(0xFF003063)
val PrimaryContainer = Color(0xFF004789)
val OnPrimaryContainer = Color(0xFFDCE2F3)
val Secondary = Color(0xFFBDC7DC)
val OnSecondary = Color(0xFF243141)
val SecondaryContainer = Color(0xFF3B4858)
val OnSecondaryContainer = Color(0xFFDAE2F9)
val Tertiary = Color(0xFFDDB0F4)
val OnTertiary = Color(0xFF3F2150)
val TertiaryContainer = Color(0xFF573868)
val OnTertiaryContainer = Color(0xFFF9D8FF)
val Error = Color(0xFFFFB4AB)
val OnError = Color(0xFF690005)
val ErrorContainer = Color(0xFF93000A)
val OnErrorContainer = Color(0xFFFFDAD6)
val Background = Color(0xFF1A1C1E)
val OnBackground = Color(0xFFE2E2E6)
val Surface = Color(0xFF1A1C1E)
val OnSurface = Color(0xFFE2E2E6)
val SurfaceVariant = Color(0xFF43474E)
val OnSurfaceVariant = Color(0xFFC3C7CF)
val Outline = Color(0xFF8D9199)
val InverseSurface = Color(0xFFE2E2E6)
val InverseOnSurface = Color(0xFF2F3033)
val InversePrimary = Color(0xFF005DB3)
val SurfaceTint = Color(0xFF9FCAFF)
val OutlineVariant = Color(0xFF43474E)
val Scrim = Color(0xFF000000)

val NightBackground = Background
val NightOverlay = SurfaceVariant
val GridLine = Primary.copy(alpha = 0.1f)
val AccentPrimary = Primary
val AccentStrong = Error
val TextPrimary = OnSurface
val TextSecondary = OnSurfaceVariant
val TextMuted = Outline
val SurfaceBorder = OutlineVariant
